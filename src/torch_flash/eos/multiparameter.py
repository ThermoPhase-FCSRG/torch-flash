"""Parameter-driven GERG/EOS-CG multiparameter Helmholtz kernel.

This module implements the shared multiparameter Helmholtz structure used by
the GERG multi-fluid approximation and the EOS-CG multiparameter mixture
model. A model is only GERG-2008 or EOS-CG when supplied with the complete,
versioned coefficient set for that model; the constructors deliberately
require such parameters instead of silently substituting a generic mixture
backend.
...
class MultiparameterMetadata:
    """Identity and validation scope of one multiparameter coefficient set.
...
class MultiparameterEOS(nn.Module):
    """Autodifferentiable multiparameter Helmholtz equation of state.
    ...
    def __init__(
        ...
        gamma_volume: Tensor,
        departure_scale: Tensor,
        metadata: MultiparameterMetadata,
        *,
        ...
        for matrix in (
            ...
            if matrix.shape != expected_pair:
                raise ValueError("all multiparameter interaction matrices must be square")
        self.names = names
    ...
    def alpha_ideal(
        ...
        if not self.has_ideal_terms:
            raise RuntimeError("this multiparameter model has no ideal Helmholtz coefficient table")
        x = normalize_composition(composition)
    ...
    def molar_volume(
        ...
        if x.shape[-1] != len(self.names):
            raise ValueError("multiparameter composition has the wrong number of components")
        ...
        if not brackets:
            raise ConvergenceError("multiparameter density solve did not converge")
        ...
        if not roots:
            raise ConvergenceError("multiparameter density solve did not find an admissible root")
        if phase == "vapor":
            density = roots[0]
        elif phase == "liquid":
            density = roots[-1]
