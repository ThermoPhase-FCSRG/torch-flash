"""Synchronize the torch-flash package version across release metadata."""
...
TARGETS = (
    ...
    VersionTarget(REPO_ROOT / "src" / "torch_flash" / "__init__.py", kind="python-version"),
...
def _replace_python_version(text: str, *, new_version: str) -> tuple[str, str]:
    ...
    if match is None:
        raise RuntimeError("Could not find __version__ assignment in src/torch_flash/__init__.py")
    current_version = match.group(2)
    updated = re.sub(
